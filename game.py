...
bg6 = Actor("bg6") # Haberler
bg6._surf = pygame.transform.scale(bg6._surf, (WIDTH, HEIGHT))
bg6.topleft = (0, 0)

bg7 = Actor("bg7") # Final TV Ekranı
try: bg7._surf = pygame.transform.scale(bg7._surf, (WIDTH, HEIGHT))
except: pass
bg7.topleft = (0, 0)

# Etkileşim Koordinatları
...
# =====================================================================
# --- DEĞİŞKENLER VE ANİMASYON SİSTEMİ ---
# =====================================================================
test_gechildi = False 
uzay_bilgileri = [
    "Uzaya gonderilen ilk canlilar maymun ya da kopek degil, 1947 yilinda ABD tarafindan gonderilen meyve sinekleridir.",
    "ISS astronotlari her 90 dakikada bir yorunge degistirdigi icin gunde 15 kez gun dogumu yasarlar.",
    ...
    "Uzay; kaynak dumani, sicak metal ve barut kokusuna benzer bir kokuya sahiptir."
]
...
# Animasyon Listeleri
p_left = ["player_left1", "player_left2", "player_left3"]
p_right = ["player_right1", "player_right2", "player_right3"]
a_left = ["astro_left1", "astro_left2", "astro_left3"]
a_right = ["astro_right1", "astro_right2", "astro_right3"]
walk_index = 0
walk_timer = 0

# Görev Durumları
atik_tamamlandi = o2_tamamlandi = jen_tamamlandi = rota_tamamlandi = False
dusman_atiklar = []
mini_oyun_skor = 0
mini_oyun_can = 3
mini_oyun_hedef = 15
...
def yeni_atik_olustur():
    ...
    atik.x = random.randint(150, WIDTH - 150); atik.y = -100
    atik.speed = random.uniform(4, 9); atik.rot_speed = random.randint(-5, 5)
...
# =====================================================================
def update():
    global mode, kozmoz_tamamlandi, intro_parlama_alfa, shake_amount, shake_x, shake_y, player
    ...
    global jen_imlec_x, jen_yon, atik_tamamlandi, o2_tamamlandi, jen_tamamlandi, rota_tamamlandi
    ...
    if guc_bari >= 100 and mode == "uzay_gemisi" and not fade_baslasin:
        fade_baslasin = True; fade_hedef = "final_sahne"
    ...
    if mode == "intro":
        ...
        moving = False
        # --- HAREKET VE ÇİFT ANİMASYON SİSTEMİ ---
        if keyboard.left or keyboard.a: 
            player.x -= 8; moving = True
            walk_timer += 1
            if walk_timer > 5:
                walk_index = (walk_index + 1) % 3
                player.image = a_left[walk_index] if test_gechildi else p_left[walk_index]
                walk_timer = 0
            ...
            if walk_timer > 5:
                walk_index = (walk_index + 1) % 3
                player.image = a_right[walk_index] if test_gechildi else p_right[walk_index]
                walk_timer = 0
        elif keyboard.up or keyboard.w: 
            player.y -= 8; moving = True
            player.image = "astro" if test_gechildi else "player"
        elif keyboard.down or keyboard.s: 
            player.y += 8; moving = True
            player.image = "astro2" if test_gechildi else "player"
        
        if not moving:
            if test_gechildi:
                if player.image in a_left: player.image = "astro3"
                elif player.image in a_right: player.image = "astro4"
            else: player.image = "player"
            walk_index = 0
            
        if player.image != eski_resim: resize_player()
        
        player.y=max(400,min(825,player.y))
        player.x=max(215,min(1225,player.x))
        if mode =="uzay_gemisi":
            player.y = max(575, min(850, player.y)); player.x = max(215, min(1225, player.x))

    elif mode == "atik_mini_oyun":
        if random.random() < 0.05: yeni_atik_olustur()
        for atik in dusman_atiklar[:]:
            atik.y += atik.speed; atik.angle += atik.rot_speed
        ...
        if mini_oyun_skor >= mini_oyun_hedef:
            guc_bari = min(100, guc_bari + 25); atik_tamamlandi = True; mode = "uzay_gemisi"; dusman_atiklar.clear()
        ...
        if time.time() - o2_gorev_vakti >= o2_basari_sure:
            guc_bari = min(100, guc_bari + 25); o2_tamamlandi = True; mode = "uzay_gemisi"
        ...
        shake_amount = 5; g_bari_val -= 0.85
        if g_bari_val <= 0: mode = "game_over"
        elif time.time() - g_baslangic_vakti >= 5: mode = "gorev_gforce_basari"
...
def draw():
    ...
    if mode == "intro":
        ...
        btn_basla = Rect((WIDTH//2 - 150, 420), (300, 70)); screen.draw.filled_rect(btn_basla, MAIN_BLUE); screen.draw.text("BAŞLA", center=btn_basla.center, fontsize=35, color=WHITE)
        btn_bilgi = Rect((WIDTH//2 - 150, 510), (300, 70)); screen.draw.filled_rect(btn_bilgi, (100,100,100)); screen.draw.text("BİLGİ", center=btn_bilgi.center, fontsize=35, color=WHITE)
        btn_cikis = Rect((WIDTH//2 - 150, 600), (300, 70)); screen.draw.filled_rect(btn_cikis, RED); screen.draw.text("ÇIKIŞ", center=btn_cikis.center, fontsize=35, color=WHITE)

    elif mode == "bilgi":
        background.draw(); overlay = pygame.Surface((1000, 600)); overlay.set_alpha(210); overlay.fill((0, 0, 0)); screen.blit(overlay, (WIDTH//2 - 500, 150))
        ...
        bg3.draw(); screen.draw.text("G KUVVETİ MAKİNESİ ", center=(1050,250), color="white", fontsize=30, owidth=1, ocolor="black")
        npc.pos = (1200 + shake_x, 750 + shake_y); npc.draw(); player.pos = (player.x + shake_x, player.y + shake_y); player.draw()
        if math.sqrt((player.x - pc_egitim_x)**2 + (player.y - pc_egitim_y)**2) < 180:
            screen.draw.text("BİLGİ [SPACE]", center=(pc_egitim_x, pc_egitim_y - 45), color="cyan", fontsize=20)
        if math.sqrt((player.x - npc.x)**2 + (player.y - npc.y)**2) < 200:
            screen.draw.text("KONUŞ [SPACE]", center=(npc.x, npc.y - 120), color="yellow", fontsize=45)

    elif mode == "uzay_gemisi":
        bg4.draw(); player.pos = (player.x + shake_x, player.y + shake_y); player.draw()
        if not rota_tamamlandi and math.sqrt((player.x - pc_gemi_x)**2 + (player.y - pc_gemi_y)**2) < 150:
            screen.draw.text("ROTA [SPACE]", center=(pc_gemi_x + 50, pc_gemi_y - 80), color="cyan", fontsize=20)
        if not o2_tamamlandi and math.sqrt((player.x - oksijen_x)**2 + (player.y - oksijen_y)**2) < 150:
            screen.draw.text("OKSİJEN [SPACE]", center=(oksijen_x + 80, oksijen_y - 100), color="white", fontsize=20)
        if not atik_tamamlandi and math.sqrt((player.x - kova_x)**2 + (player.y - kova_y)**2) < 150:
            screen.draw.text("ATIK [SPACE]", center=(kova_x, kova_y - 200), color="white", fontsize=20)
        if not jen_tamamlandi and math.sqrt((player.x - jen_panel_x)**2 + (player.y - jen_panel_y)**2) < 150:
            screen.draw.text("JENERATÖR [SPACE]", center=(jen_panel_x + 50, jen_panel_y - 80), color="orange", fontsize=20)

    elif mode == "o2_mini_oyun":
        screen.fill((10, 20, 30)); screen.draw.text("OKSİJEN DENGESİ", center=(WIDTH//2, 200), fontsize=50, color=GLOW_BLUE)
        ...
        screen.fill((20, 20, 30)); screen.draw.text("VOLTAJ SENKRONİZASYONU", center=(WIDTH//2, 200), fontsize=50, color="orange")
        ...
        screen.fill((10, 10, 25)); [atik.draw() for atik in dusman_atiklar]
        screen.draw.text(f"İMHA: {mini_oyun_skor}/15  CAN: {mini_oyun_can}", (50, 50), fontsize=40, color=WHITE)

    elif mode == "rota_mini_oyun":
        screen.fill((10, 15, 30)); screen.draw.text("AY ROTASI KALİBRASYONU", center=(WIDTH//2, 150), fontsize=60, color="cyan")
        for i in range(4):
            clr = GLOW_BLUE if i == active_digit else WHITE
            screen.draw.text(str(player_route[i]), center=(WIDTH//2-150 + i*100, 600), fontsize=80, color=clr)
            screen.draw.text(str(target_route[i]), center=(WIDTH//2-150 + i*100, 350), fontsize=80, color=GREEN)

    elif mode == "pc_okuma":
        bg3.draw(); screen.draw.filled_rect(Rect((200, 250), (WIDTH-400, 400)), (20, 20, 30))
        screen.draw.text(uzay_bilgileri[bilgi_sirasi], (250, 400), width=WIDTH-500, fontsize=35, color=WHITE)
        screen.draw.text("Kapatmak için ENTER", center=(WIDTH//2, 600), fontsize=30, color="gray")

    elif mode == "konusma":
        bg3.draw(); npc.draw(); player.draw(); screen.draw.filled_rect(Rect((100, HEIGHT-250), (WIDTH-200, 200)), (10, 10, 10))
        screen.draw.text("KAPTAN: Hazırsan G-Kuvveti testi için ENTER'a bas!", (130, HEIGHT-180), fontsize=35, color=WHITE)
     
    elif mode == "gorev_gforce":
        screen.fill((20, 0, 0)); screen.draw.text("YÜKSEK G-KUVVETİ! [SPACE] BAS!", center=(WIDTH//2 + shake_x, 200 + shake_y), fontsize=50, color=WHITE)
        screen.draw.filled_rect(Rect((WIDTH//2-200 + shake_x, 450 + shake_y), (400 * (g_bari_val/100), 50)), GREEN)
        kalan = max(0, int(5 - (time.time() - g_baslangic_vakti)))
        ...
        screen.draw.text("TEST BAŞARILI! Gemiye geçmek için ENTER", center=(WIDTH//2, HEIGHT-100), fontsize=35, color=GREEN)

    elif mode == "final_sahne":
        bg5.draw(); player.draw(); overlay = pygame.Surface((WIDTH, 150)); overlay.set_alpha(150); overlay.fill((0, 0, 0)); screen.blit(overlay, (0, 0))
        screen.draw.text("GÖREV TAMAMLANDI! AY'A İNİŞ YAPILDI!", center=(WIDTH//2, 50), fontsize=50, color=GREEN)
        screen.draw.text("Haberleri izlemek için ENTER'a bas", center=(WIDTH//2, 100), fontsize=25, color=WHITE)

    elif mode == "bg6_son":
        bg6.draw(); screen.draw.text("Sonuç ekranı için ENTER'a bas", center=(WIDTH//2, HEIGHT - 40), fontsize=20, color="gray")

    elif mode == "bg7_ekran":
        bg7.draw(); screen.draw.text("Ana menüye dönmek için ENTER'a bas", center=(WIDTH//2, HEIGHT - 40), fontsize=20, color="gray")

    elif mode == "game_over":
        screen.fill((50, 0, 0)); screen.draw.text("BAŞARISIZ! TEKRAR DENE", center=(WIDTH//2, 450), fontsize=80, color=WHITE)
        screen.draw.text("Menü için fareyle tıkla", center=(WIDTH//2, 600), fontsize=35, color="gray")

    if mode not in ["intro", "menu", "final_sahne", "bg6_son", "bg7_ekran", "bilgi"]:
    ...
    if mode == "oyun" and key == keys.SPACE:
        ...
        if key == keys.LEFT: active_digit = (active_digit - 1) % 4
            ...
            if player_route == target_route: 
                guc_bari = min(100, guc_bari + 25); rota_tamamlandi = True; mode = "uzay_gemisi"
        ...
        if WIDTH // 2 - 40 < jen_imlec_x < WIDTH // 2 + 40:
            ...
            if jen_basari_sayisi >= 3: 
                guc_bari = min(100, guc_bari + 25); jen_tamamlandi = True; mode = "uzay_gemisi"
        else: mode = "uzay_gemisi"
...
def on_mouse_down(pos):
    ...
    if mode == "menu":
        ...
        if Rect((0,0), (WIDTH, HEIGHT)).collidepoint(pos): mode = "menu"
    elif mode == "atik_mini_oyun":
        for atik in dusman_atiklar[:]:
